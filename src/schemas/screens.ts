...
Inside it, window.weave is:
  ...
  await weave.list(collection)      records, oldest first: { key, collection, body, links, createdBy, createdAt, updatedAt, mine, viaAgent }
...
export function createScreenBridge(params: {
  ...
  const collectionOf = (name: unknown) => {
    if (typeof name !== 'string' || !allowed.has(name)) throw new Error(`This screen can't use "${String(name)}"`);
    return name;
  };
  const ownRecord = async (key: unknown) => {
    if (typeof key !== 'string') throw new Error('A record key is text');
  ...
  const methods: Record<string, (...args: unknown[]) => Promise<unknown>> = {
    list: async (collection) =>
      (await node.records.list(spaceId, { collection: collectionOf(collection) })).map((record) => toScreen(record, node.did)),
    get: async (key) => {
      try {
    ...
    put: async (collection, body, options) => {
    ...
    update: async (key, body, options) => {
      const record = await ownRecord(key);
      return toScreen(await node.records.update(spaceId, record.key, plain(body, 'The record'), linksOf(options)), node.did);
    },
    remove: async (key) => {
      const record = await ownRecord(key);
